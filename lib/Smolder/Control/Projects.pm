...
sub process_add_report {
    ...
    my $report;
    eval {
        $report = $self->rs('SmokeReport')->upload_report(
    ...
    my $url = "/app/projects/smoke_reports/$project_id";
    $self->header_add(-uri => $url);
    return "Reported #$report_id added.\nRedirecting to $url";
}
...
sub smoke_report {
    ...
    my $smoke = $self->rs('SmokeReport')->find($self->param('id'));
    return $self->error_message('Smoke report does not exist')
      unless $smoke;
...
sub tap_stream {
    ...
    my $report    = $self->rs('SmokeReport')->find($report_id);
...
sub details {
    my $self = shift;
    my $id   = $self->param('id');
    my $proj = $self->rs('Project')->find($id);
...
sub mute_testfiles {
    ...
    my @testfiles = map { $self->rs('TestFile')->find($_) } @testfile_ids;
...
sub test_file_history {
    ...
    my $q            = $self->query;
    my $project      = $self->rs('Project')->find($self->param('id'));
    my $test_file    = $self->rs('TestFile')->find($self->param('test_file_id'));
